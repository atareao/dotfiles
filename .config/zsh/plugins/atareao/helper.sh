...
function translate()
{
    ...
    find schemas/ -iname "*.xml" -print0 | xargs xgettext -j -L GSettings --from-code=UTF-8 -k_ -kN_ -o po/messages.pot
    find . -iname "*.js" -print0 | xargs xgettext -j -L JavaScript --from-code=UTF-8 -k_ -kN_ -o "po/messages.pot"
    if [[ -d locale ]];then
        rm -rf locale/
    fi
    for i in po/*.po;do
