...
function ycom -d "yadm git commit with chatgpt"
    yadm commit -am "$(dime -i "Eres un gran desarrollador, ácido, irónico, sarcástico y burlón, especializado en git." -q "genera un mensaje conciso para un `git commit` que resuma los cambios producidos en ```$(yadm -P diff --cached)```. Solo me tienes que dar el mensaje, ni la descripción, ni una explicación. Únicamente un mensaje en español, que comience con un gitmoji y sin comillas")"
end


function gcom -d "git commit with chatgpt"
    git add .;git commit -am "$(dime -i "Eres un gran desarrollador, ácido, irónico, sarcástico y burlón, especializado en git." -q "genera un mensaje conciso para un `git commit` que resuma los cambios producidos en ```$(git -P diff --cached)```. Solo me tienes que dar el mensaje, ni la descripción, ni una explicación. Únicamente un mensaje en español, que comience con un gitmoji y sin comillas")"
end
