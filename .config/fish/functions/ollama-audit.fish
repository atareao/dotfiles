function ollama-audit --description 'Compara contexto actual vs máximo soportado'
    ...
    for model in (curl -s http://localhost:11434/api/tags | jq -r '.models[].name')
        ...
        set -l actual (echo $details | jq -r '.parameters' | string match -r 'num_ctx\s+(\d+)' | string split -m1 " " --fields 2 | string trim)
        ...
        if test -z "$max"; set max "Desconocido"; end
            # printf "%-25s %-12s %-12s\n" $model $actual $max
            set_color yellow; echo -n "$actual"
            set_color green; echo -n (string pad -w 10 "$max")
            set_color cyan; echo -n (string pad -w 30 "$model")
            echo
        end
